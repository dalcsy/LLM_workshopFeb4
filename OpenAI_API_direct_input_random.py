...
# 读取摘要文件的函数（从随机 5 行中任意起始点提取 50 个字符）
def load_abstracts(file_path, num_lines=5, char_limit=50):
    """从文本文件中随机选择 num_lines 行，并从任意起始点提取 char_limit 个字符"""
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            # 从每行的任意起始点提取 char_limit 个字符
            processed_abstracts = []
            for line in selected_lines:
                if len(line) > char_limit:
                    start = random.randint(0, len(line) - char_limit)
                    processed_abstracts.append(line[start:start+char_limit])
                else:
                    processed_abstracts.append(line.strip())
...
# 读取每个期刊摘要的随机 5 行，并从任意起始点提取 50 个字符
...
**Shortened Journal Abstracts (Random 5 entries, random 50 chars each):**
{journal_abstracts}
